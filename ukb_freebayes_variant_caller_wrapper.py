...
def steward(config_file_path:str, ukb_freebayes_variant_caller_path:str, send_message_script:str) -> None:
    ...
    ukb_freebayes_variant_caller_command = 'nextflow run -offline -profile singularity -bg -params-file {} {} >> run_log.txt'.format(params_file_path, ukb_freebayes_variant_caller_path)
    return_value = os.system(ukb_freebayes_variant_caller_command)
    with open('ukb_freebayes_variant_caller_command.txt', 'w') as command_f:
        command_f.write(ukb_freebayes_variant_caller_command + '\n')
    ...
    feedback_dict = {
        ...
        'pipelineName'    : 'ukb_freebayes_variant_caller',
        'analysisStatus'  : '',
                    ...
                    if 'COMPLETED' in line:
                        ...
                        report_download_dict = {
                            ...
                            'text' : [
                                {
                                    'sort'   : 1,
                                    'title'  : 'freebayes检测SNPs和Indels结果',
                                    'content': 'freebayes检测SNPs和Indels结果：#&{}'.format(vcf_gz),
                                    'postDes': '',
                                    'memo'   : '',
                                    'preDes' : ''
                                }
...
def main() -> None:
    ...
    parser.add_argument('--ukb_freebayes_variant_caller_path', required=True, help='the full path of the ukb_freebayes_variant_caller')
    ...
    ukb_freebayes_variant_caller_path = args.ukb_freebayes_variant_caller_path
    send_message_script = args.send_message_script
    steward(config_file_path, ukb_freebayes_variant_caller_path, send_message_script)
